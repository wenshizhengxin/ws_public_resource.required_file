...
class required_file extends base
{
    ...
    public function ajax_data()
    {
        try {
            ...
            if ($file_name = Args::params('file_name')) {
                $where[] = [
                    'file_name', 'like', '%' . $file_name . '%'
                ];
            }

            return $this->tableJsonData(Constant::TABLE_REQUIRED_FILE, $where, function ($row) {
                return $row;
            });
        } catch (\Exception $e) {
        }
        ...
        try {
            ...
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                $insertData = [
                    ...
                    'file_name' => Args::params('file_name/s/1'),
                ...
                if ($id === 0) { // 新增
                    ...
                    $res = Db::name('required_file')->where('id', $id)->update($insertData);
                    if (!$res) {
    ...
    public function del()
    {
        try {
            $id = Args::params('id');
            $res = Db::name('required_file')->where('id', $id)->delete();
            if (!$res) {
